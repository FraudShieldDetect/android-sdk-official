...
private constructor(
  ...
  companion object {
    ...
    fun getInstance(context: Context): FingerprintManager {
      return INSTANCE
              ?: synchronized(this) {
    ...
    fun createInstance(
      ...
      return FingerprintManager(context.applicationContext, scope)
    }
  }
  ...
  suspend fun collectFingerprintWithTimeout(timeoutMs: Long = 10000): FingerprintResult {
    return try {
                    ...
                    scope.async {
                        try {
                            ...
                            if (!collector.hasRequiredPermissions(context)) {
                                ...
                                withTimeout(3000) { // 3 second timeout per collector
  ...
  fun getAvailableCollectors(): List<String> {
    return collectors.filter { (_, collector) -> collector.isSupported(context) }.keys.toList()
  }
  ...
  fun getAllCollectorsInfo(): List<CollectorInfo> {
    return collectors.map { (_, collector) ->
      CollectorInfo(
              name = collector.getCollectorName(),
              requiredPermissions = collector.getRequiredPermissions(),
              isSupported = collector.isSupported(context),
  ...
  private fun generateFingerprintString(data: FingerprintData): String {
    return data.toJson().toString()
  }

  /** Generates fingerprint hash */
  private fun generateFingerprintHash(data: String): String {
    return try {
