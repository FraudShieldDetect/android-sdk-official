...
object SecurityCollectorUtils {
  ...
  fun readSystemProperty(property: String): String =
          try {
            val clazz = Class.forName("android.os.SystemProperties")
            val method = clazz.getMethod("get", String::class.java, String::class.java)
            (method.invoke(null, property, "") as? String)?.trim() ?: ""
          } catch (e: Exception) {
            ""
          }
  ...
  fun canWriteToDirectory(dir: String): Boolean =
          try {
  ...
  fun readNetworkInterfaces(): List<String> =
          try {
  ...
  fun readGlobalSetting(context: Context, key: String): Int =
          try {
  ...
  fun getEncryptionStatusName(status: Int): String =
          when (status) {
            ...
            android.app.admin.DevicePolicyManager.ENCRYPTION_STATUS_ACTIVE_DEFAULT_KEY ->
                    "ACTIVE_DEFAULT_KEY"
